...
module.exports = async (client, oldMember, newMember) => {
    ...
    if (settings.streamroleEnabled === "true") {
        const verified = newMember.guild.roles.find('name', settings.verifiedRole);
    ...
    if (settings.streamshoutEnabled === "true") {
        ...
        if (!channel) {
            console.log(`ERR: channel with key 'streamChannel' was not found`);
            return;
        }
        if (!newMember.presence.game) {
            return;
            ...
            request(options, function (error, response, body) {
                ...
                if (strem.game_id === "503116"){
                    if (newMember.presence.game.streaming && oldMember.presence.game.streaming) {
                        console.log(`already streming ${newMember.user.tag}`);
                        return;
                    }
